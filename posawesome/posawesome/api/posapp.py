...
@frappe.whitelist()
def get_items(pos_profile, price_list=None, item_group="", search_value="", barcode_search_type="AUTO"):
...
def get_barcode_item_details(item_code, pos_profile, price_list):
    pos_profile = json.loads(pos_profile)
    today = nowdate()
    posa_display_items_in_stock = pos_profile.get("posa_display_items_in_stock")
    search_serial_no = pos_profile.get("posa_search_serial_no")
...
def get_seearch_items_conditions(item_code, serial_no, batch_no, barcode):
    if serial_no or batch_no or barcode:
        return " and itm.name = {0}".format(frappe.db.escape(item_code))
    return """ and (itm.name like {item_code} or itm.item_name like {item_code})""".format(
