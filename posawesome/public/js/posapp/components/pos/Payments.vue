      ...
      <div class="overflow-y-auto px-2 pt-2" style="max-height: 75vh">
        <v-row v-if="invoice_doc" class="px-1 py-0">
          <v-col cols="7">
            <v-text-field outlined color="primary" :label="frappe._('Paid Amount')" background-color="white" hide-details
              :model-value="formtCurrency(total_payments)" readonly :prefix="currencySymbol(invoice_doc.currency)"
              dense></v-text-field>
          </v-col>
          <v-col cols="5">
        ...
        <div v-if="is_cashback">
          <v-row class="pyments px-1 py-0" v-for="payment in invoice_doc.payments" :key="payment.name">
            <v-col cols="6" v-if="!is_mpesa_c2b_payment(payment)">
              <v-text-field v-model="payment.amount" density="compact" variant="outlined" color="primary"
                :label="frappe._(payment.mode_of_payment)" bg-color="white" hide-details
                :model-value="formtCurrency(payment.amount)"
                @update:model-value="setFormatedCurrency(payment, 'amount', null, true, $event)" :rules="[isNumber]"
        ...
        <v-row class="pyments px-1 py-0" v-if="invoice_doc &&
          ...
          <v-col cols="5">
            <v-text-field dense outlined color="primary" :label="frappe._('You can redeem upto')" background-color="white"
              hide-details :model-value="formtFloat(available_pioints_amount)"
        ...
        <v-row class="px-1 py-0">
          ...
          <v-col v-if="invoice_doc.rounded_total" cols="6">
            <v-text-field dense outlined color="primary" :label="frappe._('Rounded Total')" background-color="white"
              hide-details :model-value="formtCurrency(invoice_doc.rounded_total)" disabled
              :prefix="currencySymbol(invoice_doc.currency)"></v-text-field>
          </v-col>
          <v-col cols="6" v-if="pos_profile.posa_allow_sales_order && invoiceType == 'Order'">
            <v-menu v-model="order_delivery_date" :close-on-content-click="false" transition="scale-transition">
              <template #activator="{ props }">
                <v-text-field v-model="invoice_doc.posa_delivery_date" v-bind="props" :label="frappe._('Delivery Date')"
                  readonly variant="outlined" clearable density="compact" hide-details="auto" class="bg-white"
                  color="primary"></v-text-field>
              </template>

              <v-date-picker v-model="invoice_doc.posa_delivery_date" color="primary" :min="frappe.datetime.now_date()"
                @update:model-value="order_delivery_date = false"></v-date-picker>
            </v-menu>
          </v-col>
        ...
        <v-row class="px-1 py-0" align="start" no-gutters>
          ...
          <v-col cols="6" v-if="is_credit_sale">
            <v-menu v-model="date_menu" :close-on-content-click="false" transition="scale-transition">
              <template #activator="{ props }">
              ...
              <v-date-picker v-model="invoice_doc.due_date" show-adjacent-months color="primary"
                :min="frappe.datetime.now_date()" @update:model-value="() => (date_menu = false)" />
            </v-menu>
          </v-col>
          ...
          <v-row v-for="(row, idx) in customer_credit_dict" :key="idx">
            ...
            <v-col cols="4">
              <v-text-field dense outlined color="primary" :label="frappe._('Available Credit')" background-color="white"
                hide-details :model-value="formtCurrency(row.total_credit)" disabled
                :prefix="currencySymbol(invoice_doc.currency)"></v-text-field>
            </v-col>
            <v-col cols="4">
...
export default {
  ...
  methods: {
    ...
    submit(event, payment_received = false, print = false) {
      ...
      if (!payment_received && this.invoiceType != "Order") {
    ...
    load_print_page() {
      ...
      const url =
        ...
        `/printview?doctype=${doctype}&name=` +
  ...
  mounted: function () {
    this.$nextTick(function () {
      evntBus.$on("send_invoice_doc_payment", (invoice_doc) => {
        ...
        this.is_credit_sale = (
          this.invoiceType == "Order" ? true : false
        );
